...
function getDemocraticPresidents(presidentsArr) {
  return presidentsArr.filter(
    (DemocraticPresidents) => DemocraticPresidents.party === "Democratic"
  );
}

// Iteration 3 | Count Years in Office - reduce()
function countYearsInOffice(presidentsArr) {
  return presidentsArr.reduce((acc, yearsInOffice) => {
    if (yearsInOffice.leftOffice === null) return acc;
    acc += yearsInOffice.leftOffice - yearsInOffice.tookOffice;
    return acc;
  }, 0);
}
...
function sortPresidentsByBirthYear(presidentsArr) {
  presidentsArr.sort(function (president1, president2) {
...
function getAgeAtInauguration(presidentsArr) {
  return presidentsArr.map(
    (president) => president.tookOffice - president.birthYear
  );
}
...
function countRepublicanPresidents(presidentsArr) {
  return presidentsArr.reduce((acc, republican) => {
    acc += presidentsArr - republican;
  }, 0);
}
